...
async function run() {
  ...
  try {
    const seeded = await connection.begin(async (tx) => {
      ...
      await insert(sql`INSERT INTO "company_channels" ("id", "platform", "platform_id", "company_id", "status", "metadata", "ai_disabled") VALUES
        ('seed-chan-starter', '${ChannelPlatform.WHATSAPP}'::"ChannelPlatform", '2348012345678', 'seed-co-starter', '${CompanyChannelStatus.ACTIVE}'::"CompanyChannelStatus", '{"verified":true}'::jsonb, false),
        ('seed-chan-pro', '${ChannelPlatform.WHATSAPP}'::"ChannelPlatform", '2348098765432', 'seed-co-pro', '${CompanyChannelStatus.ACTIVE}'::"CompanyChannelStatus", '{"verified":true}'::jsonb, false)
        ON CONFLICT ("id") DO NOTHING`);
      ...
      // Unified metrics table (supersedes company_daily/company_lifetime_metrics):
      // one row per (company_id, domain, granularity, bucket_start); NULL
      // company_id = system-wide bucket; LIFETIME rows use LIFETIME_BUCKET_START.
      await insert(sql`INSERT INTO "metrics"
        ("company_id", "domain", "granularity", "bucket_start", "total_count", "delivered_count", "cancelled_count", "failed_count", "avg_delivery_time_minutes", "channel_breakdown", "peak_hour", "unique_riders_active") VALUES
        ('seed-co-starter', '${MetricDomain.DELIVERIES}'::"MetricDomain", '${MetricGranularity.DAY}'::"MetricGranularity", CURRENT_DATE, 5, 2, 0, 0, 45, '{"WHATSAPP":5}'::jsonb, 14, 2),
        ('seed-co-pro', '${MetricDomain.DELIVERIES}'::"MetricDomain", '${MetricGranularity.DAY}'::"MetricGranularity", CURRENT_DATE, 3, 1, 1, 0, 60, '{"WHATSAPP":3}'::jsonb, 11, 1),
        (NULL, '${MetricDomain.DELIVERIES}'::"MetricDomain", '${MetricGranularity.DAY}'::"MetricGranularity", CURRENT_DATE, 1, 0, 0, 0, NULL, '{}'::jsonb, NULL, 0)
        ON CONFLICT ("company_id", "domain", "granularity", "bucket_start") DO NOTHING`);

      await insert(sql`INSERT INTO "metrics"
        ("company_id", "domain", "granularity", "bucket_start", "total_revenue_kobo") VALUES
        ('seed-co-starter', '${MetricDomain.REVENUE}'::"MetricDomain", '${MetricGranularity.DAY}'::"MetricGranularity", CURRENT_DATE, 1500000),
        ('seed-co-pro', '${MetricDomain.REVENUE}'::"MetricDomain", '${MetricGranularity.DAY}'::"MetricGranularity", CURRENT_DATE, 970000),
        (NULL, '${MetricDomain.REVENUE}'::"MetricDomain", '${MetricGranularity.DAY}'::"MetricGranularity", CURRENT_DATE, 0)
        ON CONFLICT ("company_id", "domain", "granularity", "bucket_start") DO NOTHING`);

      await insert(sql`INSERT INTO "metrics"
        ("company_id", "domain", "granularity", "bucket_start", "total_count", "delivered_count", "total_revenue_kobo", "channel_breakdown") VALUES
        ('seed-co-starter', '${MetricDomain.DELIVERIES}'::"MetricDomain", '${MetricGranularity.LIFETIME}'::"MetricGranularity", '${LIFETIME_BUCKET_START}', 25, 10, 4500000, '{"WHATSAPP":25}'::jsonb),
        ('seed-co-pro', '${MetricDomain.DELIVERIES}'::"MetricDomain", '${MetricGranularity.LIFETIME}'::"MetricGranularity", '${LIFETIME_BUCKET_START}', 18, 9, 12000000, '{"WHATSAPP":18}'::jsonb),
        (NULL, '${MetricDomain.DELIVERIES}'::"MetricDomain", '${MetricGranularity.LIFETIME}'::"MetricGranularity", '${LIFETIME_BUCKET_START}', 40, 30, 12000000, '{}'::jsonb)
        ON CONFLICT ("company_id", "domain", "granularity", "bucket_start") DO NOTHING`);
