...
## Schema is two files, destroy-and-rebuild only

`drizzle/0000_initial.sql` (generated by Drizzle from `schema.ts`, enum members manually maintained at the top of the file) contains **only** `CREATE TYPE`/`CREATE TABLE`/indexes/constraints/RLS and ends with an `APPENDIX` sentinel. `sql/appendix.sql` (hand-owned, append-only) contains extensions (`pg_cron`, `pgmq`), functions, cron jobs, queues, grants, and the `billing_notifications` queue contract. Both files are idempotent.

**Recreate command:** `npm run db:recreate` (or `npx tsx scripts/recreate.ts`) — runs `DROP SCHEMA public CASCADE` → applies `0000_initial.sql` → applies `sql/appendix.sql` → seeds tracking table + drizzle journal. Requires `--yes` or `LOGISTIX_RECREATE=yes` env to confirm. Uses postgres-js, same driver/SSL/localhost handling as `scripts/migrate.ts`.

**When changing the schema:** edit `schema.ts`, then `npx drizzle-kit generate` in the drizzle scratch config to update `0000_initial.sql` (re-run `npm run db:recreate` against local dev DB to validate). If you add or change enum members, they **must** be manually added to the top of `0000_initial.sql` (the generated file's `DROP TYPE IF EXISTS` + `CREATE TYPE` block); the drift test (`recreate-drift.test.ts`) guards that `IN_TRANSIT`/`SUSPENDED` and all other members are present.

**When adding cron jobs, functions, or queues:** append to `sql/appendix.sql` only. Functions use `CREATE OR REPLACE FUNCTION`; queues use `pgmq.create`; cron jobs use `cron.schedule` (upsert-style). The `appendix-drift.test.ts` tests guard billing notification reason strings.

**Never push `0000_initial.sql` or `sql/appendix.sql` out of sync with `schema.ts`.** The `recreate-drift.test.ts` guards that the generated file has no hand-owned DDL (CREATE EXTENSION, cron.schedule, pgmq.create, CREATE OR REPLACE FUNCTION pg_cron, GRANT ALL) and ends with the APPENDIX sentinel.
