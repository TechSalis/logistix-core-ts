...
- **⚠️ PIN-BUMP DONE (release):** consumers bumped `547c639` → `fed0e4c` (backend `f0f6cc7b`, workers `671617c`, web `9306ebd`, manifests + lockfiles, zero stale SHAs). ⚠️ **core-ts HEAD `fed0e4c` is still local-only** (main is ahead of origin) — the `github:#` pin cannot resolve in a fresh install/CI until core-ts is pushed. Push core-ts to complete the release.
